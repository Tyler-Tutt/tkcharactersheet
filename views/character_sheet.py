...
class CharacterSheet(PageBase):
    ...
    def build_ui(self):
        ...
        # container.grid(self, column=0, row=0)
        ...
        stats_frame.grid(column=0, row=1, sticky="nsew")
        ...
        middleframe.columnconfigure(0, weight=1) # Allow content to fill the column
        middleframe.grid(column=1, row=1, sticky='nsew', padx=10)

        # --- AC, Initiative, Speed Frame ---
        ac_init_speed_frame = ttk.Frame(middleframe)
        ac_init_speed_frame.columnconfigure((0,1,2), weight=1)
        ac_init_speed_frame.grid(column=0, row=0, sticky='ew', pady=(0, 10))

        # Armor Class
        ttk.Entry(ac_init_speed_frame, textvariable=self.character.char_vars['armor_class'], justify='center').grid(column=0, row=0, sticky='ew')
        ttk.Label(ac_init_speed_frame, text="Armor Class", anchor='center').grid(column=0, row=1)

        # Initiative
        ttk.Entry(ac_init_speed_frame, textvariable=self.character.char_vars['initiative'], justify='center').grid(column=1, row=0, sticky='ew', padx=5)
        ttk.Label(ac_init_speed_frame, text="Initiative", anchor='center').grid(column=1, row=1)

        # Speed
        ttk.Entry(ac_init_speed_frame, textvariable=self.character.char_vars['speed'], justify='center').grid(column=2, row=0,sticky='ew')
        ttk.Label(ac_init_speed_frame, text="Speed", anchor='center').grid(column=2, row=1)
